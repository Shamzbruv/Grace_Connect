...
serve(async (req) => {
  ...
  try {
    ...
    if (RESEND_API_KEY) {
      const itemsListText = cart.map((item: any, idx: number) => `${idx + 1}. ${item.name} × ${item.quantity} — J$${(item.price * item.quantity).toLocaleString()}`).join('\n')
      ...
      const customerHtml = `
        ...
        <b>Shipping:</b> J$${shippingCost.toLocaleString()}<br>
        <b>Total:</b> J$${total.toLocaleString()}</p>
      ...
      const ownerHtml = `
        ...
        Shipping: J$${shippingCost.toLocaleString()}<br>
        Total: J$${total.toLocaleString()}</p>
      ...
      await supabaseAdmin.from('email_logs').insert({
        ...
        resend_email_id: resCustomerJson.id || null,
        status: resCustomer.ok ? 'sent' : 'error',
        error_message: resCustomer.ok ? null : JSON.stringify(resCustomerJson)
      })
    }
