-- Same-church members should be able to message each other once both are
-- active in the church. Bible Nudge grants remain required for cross-church
-- messaging when no prior relationship exists.
...
begin
  ...
  other_is_same_church := coalesce(other_member."placeId", '') = actor_church_id;
  ...
  if not other_is_same_church and not has_message_grant then
  ...
  if not other_is_same_church
      and not coalesce(other_member."allowMessages", true)
  ...
  with check (
    ...
    and (
      ...
      where u.uid = any(member_ids)
        and (
          u."placeId" = public.get_church_id()
          or coalesce(u."allowMessages", true)
