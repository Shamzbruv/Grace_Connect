-- Global rankings show first names only.
--
-- Two opposite problems, same cause -- neither board was written with
-- strangers in mind:
--   * The streak board published a member's full legal name to every user of
--     the app, worldwide. Inside one church that is a familiar face; on a
--     global board it is a stranger's full name next to their reading habits.
--   * The quiz board went the other way and showed "Member" for everyone,
--     which is private but useless -- nobody can find themselves.
-- A first name is recognisable to people who know you and not identifying to
-- people who do not. Church-scoped boards keep full names, since those are
-- people who already know each other.

create or replace function private.display_first_name(full_name text)
...
begin
  ...
  with scored as (
    ...
    select
      ...
      case
        when scope = 'global' and s.member_id <> actor
          then private.display_first_name(
            coalesce(nullif(btrim(u."displayName"), ''), u."fullName"))
        else coalesce(nullif(btrim(u."displayName"), ''),
